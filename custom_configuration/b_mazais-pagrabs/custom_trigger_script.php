...
function process_trigger_1()
 ...
 error_log ("@@@@@@@@@@@@@@@@@@Trigger parameters: pagrabs_T $pagrabs_T   aara_T   $aara_T  merka_T $merka_T   delta_T_merka  $delta_T_merka   delta_T_pagrabs_aara  $delta_T_pagrabs_aara");
 
 
  if ( !is_null($pagrabs_T) AND  !is_null($aara_T) ) {
      if  ($pagrabs_T >= ($merka_T + $_delta)) { // vai vajag dzeset? 
            if  ($pagrabs_T >= ($aara_T + $_delta)) { // vai var dzeset?
                    $ventolators_action  = 1; // on
              }
              elseif  ($pagrabs_T <= ($aara_T - $_delta)) {
                    $ventolators_action  = 0; // off
              }
      }
      elseif  ($pagrabs_T <= ($merka_T - $_delta)) {
            $ventolators_action  = 0; // off
      }
  }
